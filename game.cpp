...
simulate_game(Input* input, float dt) {
	draw_rect(0, 0, arena_half_size_x, arena_half_size_y, 0x7CFC00);
	draw_arena_borders(arena_half_size_x, arena_half_size_y, 0x06402B);
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		// Simulate Ball
		{
			...
			if (ball_p_x + ball_half_size > arena_half_size_x) {
				...
				ball_p_y = 0;
				player_1_score++;
				if (player_1_score >= 20) {  // Check for game over condition
				...
				ball_p_y = 0;
				player_2_score++;
				if (player_2_score >= 20) {  // Check for game over condition
					current_gamemode = GM_GAMEOVER;
				}
			}
		}
	...
	{
		draw_text("GAME OVER", -73, 40, 2, 0xffffff);
		if(player_1_score > player_2_score)
		draw_text("PLAYER ONE WINS", -73, 22, .75, 0xffffff);
		else
		draw_text("PLAYER TWO WINS", -73, 22, .75, 0xffffff);

		draw_text("PRESS ENTER TO RETURN TO MENU", -73, 15, 1.22, 0xffffff);
		draw_text("PRESS ESC TO EXIT", -73, 5, 1.22, 0xffffff);
		...
		if (pressed(BUTTON_ESC)) {
			PostQuitMessage(0);  // Windows-specific function to exit the application
		}
	}
	else {
		...
		if (hot_button == 0) {
			draw_text("PLAY WITH AI", -80, -10, 1, 0xff0000);
		...
		draw_text("AI AND MULTI", -73, 40, 2, 0xffffff);
		draw_text("PLAYER PONG GAME", -73, 22, .75, 0xffffff);
		draw_text("JASMINE", -73, 15, 1.22, 0xffffff);
	}
}